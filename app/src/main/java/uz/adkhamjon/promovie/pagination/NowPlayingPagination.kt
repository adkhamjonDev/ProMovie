...
class NowPlayingPagination @Inject constructor(val apiService: ApiService): PagingSource<Int, MovieClass>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MovieClass> {
        try {
            ...
            val movie = apiService.getNowPlaying(page = nextPageNumber)
            if (nextPageNumber > 1) {
